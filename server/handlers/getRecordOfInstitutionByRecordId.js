...
const getRecordOfInstitutionByRecordId = async (request, response) => {
    const client = new MongoClient(MONGO_URI, options);

    try {

        const { institutionName, recordId } = request.params;

        await client.connect();
        const db = client.db("OpenArchive");
        const recordResult = await db.collection("records").findOne({"repository": institutionName, "_id": recordId});
...
module.exports = getRecordOfInstitutionByRecordId;